...
public:
    ...
    vector<vector<size_t>> get_stats() const {
        vector<vector<size_t>> output;
        for (auto &p: pgms) {
            vector<size_t> p_stat;
            p_stat.push_back(p.segments_count());
            p_stat.push_back(p.height());
            p_stat.push_back(p.size_in_bytes());
            output.push_back(p_stat);
        }
