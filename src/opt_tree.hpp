#ifndef OPTBPLUSTREE_HPP
#define OPTBPLUSTREE_HPP
...
#include "utils/linear_regression.hpp"

#ifdef _OPENMP
#pragma message("Compile opt_tree.hpp with OpenMP")
#include <omp.h>
#else
...
#define RETRAIN_THRESHOLD (FANOUT * 2)

// #define NODEBUG
template <typename key, typename val>
using dense_hashmap = ankerl::unordered_dense::map<key, val>;

static ThreadPool thread_pool(MAX_THREAD_NUM);
...
public:
    ...
    struct InnerNode : Node {
        ...
        std::vector<key_type> buffer_slotkey;
        std::vector<Node*> buffer_child_nodes;
        ...
        void retrain() {
            auto model = LinearRegression<uint64_t, uint32_t>();
            for (int i = 0; i < slotkey.size(); i++) {
                model.addDataPoint(slotkey[i], i);
            }
            auto [s, i] = model.computeModel();
            this->slope = s;
            this->intercept = i;
            this->error = model.computeMaxError(s, i);
            if (this->error < this->slotkey.size() / 2) {
                is_model = true;
            }
        }
    ...
    struct LearnedLeafNode : LeafNode {
        ...
        dense_hashmap<key_type, int32_t> buffer_index;
        ...
        LearnedLeafNode() : LeafNode() {
            buffer_slotdata.reserve(RETRAIN_THRESHOLD);
            buffer_index.reserve(RETRAIN_THRESHOLD * 2);
        ...
        void insert_data_to_buffer(kv_pair& kv) {
            buffer_index[kv.first] = buffer_slotdata.size();
        ...
        bool key_exist(const key_type& key) override {
            ...
            return this->slotdata[model_idx] == key || buffer_index[model_idx] != -1;
        }
...
public:
    std::vector<key_type> range_search(const key_type& start_key, const key_type& end_key) {
        std::vector<key_type> results;
        key_type masked_start_key = start_key << 1;
        key_type masked_end_key = end_key << 1;
        Node* n = root;
        if (!n) return results;
        ...
        int cnt = 0;
        while (leaf) {
            bool is_this_leaf = leaf->slotdata[leaf->slotdata.size() - 1] > masked_end_key;
            int to_copy;
            if (is_this_leaf) {
                if (leaf->is_model_leafnode()) {
                    LearnedLeafNode* learned_leaf = static_cast<LearnedLeafNode*>(leaf);
                    int q = learned_leaf->search_model(masked_end_key);
                    to_copy = q - i + 1;
                    std::memcpy(results.data() + cnt, learned_leaf->slotdata.data() + i, to_copy * sizeof(key_type));
                    cnt += to_copy;
                    std::memcpy(results.data() + cnt, learned_leaf->buffer_slotdata.data(), learned_leaf->buffer_slotdata.size() * sizeof(key_type));
                    cnt += learned_leaf->buffer_slotdata.size();
                } else {
                    int q = find_lower(leaf, masked_end_key);
                    to_copy = q - i + 1;
                ...
                if (leaf->is_model_leafnode()) {
                    LearnedLeafNode* learned_leaf = static_cast<LearnedLeafNode*>(leaf);
                    to_copy = learned_leaf->slotdata.size() - i;
                    std::memcpy(results.data() + cnt, learned_leaf->slotdata.data() + i, to_copy * sizeof(key_type));
                    cnt += to_copy;
                    std::memcpy(results.data() + cnt, learned_leaf->buffer_slotdata.data(), learned_leaf->buffer_slotdata.size() * sizeof(key_type));
                    cnt += learned_leaf->buffer_slotdata.size();
                } else {
                    to_copy = leaf->slotdata.size() - i;
                    std::memcpy(results.data() + cnt, leaf->slotdata.data() + i, to_copy * sizeof(key_type));
                    cnt += to_copy;
                }
            }
        ...
        const int q = array_lower_bound(results.data(), results.size(), masked_end_key);
        results.resize(q + 1);
        return results;
    }
    ...
    bool insert(kv_pair& key_value) {
        ...
        if (!leaf->is_model_leafnode()) {
            ...
            // TODO: Implement the insertion for model-based leaf node
            auto learned_leaf = static_cast<LearnedLeafNode *>(leaf);
            learned_leaf->insert_data_to_buffer(kv);
            if (learned_leaf->buffer_size() >= learned_leaf->buffer_retain_threshold) {
                thread_pool.enqueue([this, learned_leaf]() {
                    this->detect_and_retrain_model_leaf(learned_leaf);
                });
            }
        }
...
public:
    ...
    bool erase(key_type& original_key) {
        ...
        if (!leaf->is_model_leafnode()) {
            ...
            // TODO: Implement the handle underflow of model-based leaf node
            auto* learned_leaf = static_cast<LearnedLeafNode*>(leaf);
            ...
            if (learned_leaf->slotdata[slot] == key) {
                ...
                auto buffer_idx = learned_leaf->buffer_index[key];
                learned_leaf->buffer_index[key] = -1;
                // learned_leaf->buffer_index.erase(key);
                auto tmp = learned_leaf->buffer_slotdata[buffer_idx];
                learned_leaf->buffer_slotdata[buffer_idx] = learned_leaf->buffer_slotdata[learned_leaf->buffer_slotdata.size() - 1];
                learned_leaf->buffer_slotdata.resize(learned_leaf->buffer_slotdata.size() - 1);
            }
        }
...
private:
    Node* copy_node(Node* node) {
        ...
        if (node->is_model_leafnode()) {
            ...
            new_leaf->buffer_index = leaf->buffer_index;
...
public:
    std::pair<std::vector<Node *>*, std::vector<Node *>*> get_retrain_path_snapshot(LearnedLeafNode* node) {
        // TODO: Locking path when snapshoot subtree
